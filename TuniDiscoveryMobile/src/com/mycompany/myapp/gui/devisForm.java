 ...
 * @author bhk
 */
public class devisForm extends Form{

     Form current;
     Resources res;
     int idr;
     Reservation resinser;
     SimpleDateFormat formater = new SimpleDateFormat("yyyy-MM-dd");
  
       public devisForm(Form previous) {
           
           //récupération de la dernière réservation insérée
             idr=0;
            for ( Reservation reserv:ServiceReservation.getInstance().getAllReservation())
                idr=reserv.getId();
            for ( Reservation reservch:ServiceReservation.getInstance().getAllReservation())
                if(reservch.getId()==idr)
                    resinser=reservch;
